...
![Token Mask Caching: Validación O(1) por Token en Tiempo de Ejecución](./diagrams/token_mask_caching.png)

> **Token Mask Caching: Validación O(1) por Token en Tiempo de Ejecución**
>
> El diagrama muestra dos fases. **Izquierda (compilación):** se precalcula un bitmap `mask[q]` por estado usando `mask[q] |= (1 << t)` para cada terminal `t` con transición válida. Ejemplo: q1 con `{'+', '*', ')'}` → `mask[q1] = 0b001110`. **Derecha (ejecución):** para validar el token `'+'` (id=1) en estado q1, basta una operación `mask[q1] & (1<<1) ≠ 0` → válido; para `'ID'` (id=0) el AND da 0 → inválido. **Abajo (estrategia adaptativa):** XGrammar elige automáticamente entre `u64` bitmask (|Σ| ≤ 64), `u64[4]` array (|Σ| ≤ 256), o `HashSet` (vocabularios grandes como LLMs con ~50k tokens).
...
![Análisis de alcanzabilidad DFA: antes con estados muertos, después limpio](./diagrams/alcanzabilidad_dfa.png)

> **Eliminación de Estados Muertos mediante Alcanzabilidad**
>
> El algoritmo aplica dos BFS: uno hacia adelante desde q0 para marcar estados alcanzables, y uno hacia atrás desde los estados de aceptación para marcar estados útiles. Los estados que no pasan ambas pruebas se eliminan: q3 y q4 (no alcanzables desde q0), q5 (alcanzable pero sin camino a ningún estado de aceptación). El DFA resultante es equivalente pero más compacto.
