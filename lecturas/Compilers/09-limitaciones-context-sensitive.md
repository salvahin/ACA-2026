...
![Jerarquía de Chomsky: Clases de Lenguajes Formales](./diagrams/chomsky_jerarquia.png)

> **Jerarquía de Chomsky: Clases de Lenguajes Formales**
>
> Diagrama de rectángulos anidados (de mayor a menor poder expresivo): **Tipo 0** (ámbar) agrupa todos los lenguajes Turing-computables con reglas sin restricción α→β. Dentro, **Tipo 1** (rojo) muestra los Context-Sensitive procesados por autómatas linealmente acotados con reglas αAβ→αγβ. Más adentro, **Tipo 2** (índigo) representa los Context-Free reconocidos por PDAs con reglas A→β — aquí opera XGrammar usando BNF/EBNF para constrained decoding. Al centro, **Tipo 3** (verde) son los lenguajes Regulares reconocidos por DFA/NFA con reglas A→aB. La nota inferior confirma la relación de subconjunto estricto: Tipo 3 ⊂ Tipo 2 ⊂ Tipo 1 ⊂ Tipo 0.
...
![Análisis multi-pasada: cuatro fases de compilación mapeadas a la jerarquía de Chomsky](./diagrams/analisis_multi_pasada.png)

> **Análisis Multi-Pasada — Cuatro Fases y su Nivel de Chomsky**
>
> La estrategia multi-pasada asigna cada fase al nivel de gramática que puede manejarla: Lexing usa autómatas finitos (Tipo 3), Parsing usa CFG y PDA (Tipo 2), el Análisis Semántico requiere context-sensitive (Tipo 1) para verificar tipos y declaraciones, y la Generación de Código usa lógica irrestricta (Tipo 0). Esta separación permite optimizar cada fase independientemente.
