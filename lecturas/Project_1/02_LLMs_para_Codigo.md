...
![Matriz de selección de LLM: trade-offs entre calidad, velocidad y costo](./diagrams/llm_selection_matrix.png)

> **Selección de LLM para Código — Matriz de Trade-offs**
>
> No existe un modelo universalmente mejor: la elección depende del volumen de tokens, la tolerancia a latencia, los requisitos de privacidad y el presupuesto. Modelos pequeños (DeepSeek 1.5B, Qwen 7B) son ideales para desarrollo rápido y prototipado; modelos grandes vía API (Claude, GPT-4o) maximizan calidad cuando el costo por token es tolerable; self-hosted (DeepSeek 33B) es el punto óptimo para producción con alto volumen.
